...
void DrawMultiMesh(Mesh mesh, Material material, Matrix transform, const int *count, const void *const *indices, int drawcount)
{
#if defined(GRAPHICS_API_OPENGL_11)
    #define GL_VERTEX_ARRAY         0x8074
#define GL_NORMAL_ARRAY         0x8075
...
#define GL_TEXTURE_COORD_ARRAY  0x8078
    ...
    for (int eye = 0; eye < eyeCount; eye++)
    {
        ...
        //if (mesh.indices != NULL) rlDrawVertexArrayElements(0, mesh.triangleCount*3, 0);
        if (mesh.indices != NULL)
            rlMultiDrawVertexArrayElements(count, indices, drawcount);
        else rlDrawVertexArray(0, mesh.vertexCount);
    }
...
void DrawMultiModel(Model model, Vector3 position, float mono_scale, Color tint, const int *count, const void *const *indices, int drawcount)
{
    Vector3 scale = { mono_scale, mono_scale, mono_scale };
    Vector3 rotationAxis = { 0.0f, 1.0f, 0.0f };
    ...
    for (int i = 0; i < model.meshCount; i++)
    {
        ...
        DrawMultiMesh(model.meshes[i], model.materials[model.meshMaterial[i]], model.transform, count, indices, drawcount);
